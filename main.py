...
voting_classifler_model = load_model('voting_clf.pkl')

xgboost_SMOTE_model = load_model('xgboost-SMOTE.pkl')

xboost_featureEngineered_model = load_model('xgboost-featureEngineered.pkl')
...
def prepare_input(credit_score, location, gender, age, tenure, balance,
  ...
  input_dict = {
      ...
      'Geography _Germany': 1 if location == 'Germany' else 0,
...
def explain_prediction(probability, input_dict, surname):
  prompt = f"""You are an expert data scientist at a bank, where you specialize in interpreting and Lexplaining predictions of machine learning models.
  
  Your machine learning model has predicted that a customer named {surname} has a {round (probability *100, 1)}% probability of churning, based on the information provide below.
...
def generate_email(probability, input_dict, explanation, surname):
    ...
    Make sure to list out a set of incentives to stay based on their information, in bullet point format. Don't ever mention the probability of_churning or the machine learning model to the customer.
    """
    raw_response = client.chat.completions.create(model="llama-3.1-8b-instant", messages=[{"role": "user","content": prompt}],)
